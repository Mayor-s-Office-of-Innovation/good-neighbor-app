...
- [~] **Implement the analysis backend Lambdas** (per the plan written above): presigned-PUT upload to GNP's bucket → **async worker** reads S3, downscales + base64, per-artifact analyzer call with the API key from Secrets Manager → adapt + persist scorecard + S3 key; perimeter-check CRUD/list endpoints; presigned-GET for admin review; DynamoDB read/write. **Step A built + green (2026-08-14):** pure/offline `backend/src/analysis/` — `contract.js` (vendored response types + `RUBRIC_ID`/`RUBRIC_VERSION`), `rubric-meta.js` (versioned 13-category weighting map + `weightingFor`), `adapt-scorecard.js` (`adaptAssessment` → per-artifact `concerns[]`, grade from the service, unknown-category flagging), `synthesize-check.js` (`synthesizeCheck` → worst-grade + per-category max rollup, shaped for the `CHECK#` header), GNA fixtures + dependency-free Vitest tests (25 pass; typecheck + lint clean). **Remaining:** B analyzer client (stubbable) → C handlers/worker + `classify-task` escalation seam → D local harness (MinIO) → E Terraform → F live E2E (only F waits on the analyzer being deployed + a GNP key).
