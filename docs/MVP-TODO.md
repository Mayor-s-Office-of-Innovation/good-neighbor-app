...
- [~] **Stand up the Docker-free local harness** (in-process router, ElasticMQ, DynamoDB Local, bootstrap/worker/GUI scripts, one-command `npm run dev`). **Built (2026-08-13):** `backend/scripts/` — `local-ddb`/`local-mq` (JVM jars), `lib/ensure-infra` (idempotent table+queue create, shape mirrors Terraform), `local-api` (in-process router → real handlers, stub `sub`), `local-worker` (long-poll pump → real worker), `db:gui` (dynamodb-admin); `.env.example` + `--env-file`; `npm run dev -w backend`. Node-only pieces verified (syntax + `buildProxyEvent` unit test). **Remaining:** live end-to-end run — needs **JRE 17+** installed locally (S3/MinIO still deferred; handlers don't touch S3 yet). — [local-dev plan](./local-dev-environment-plan.md)
