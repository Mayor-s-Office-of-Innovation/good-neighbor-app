...
async function upsertSite(docDdb, tableName, seed, now) {
  await docDdb.send(
    new UpdateCommand({
      ...
      UpdateExpression:
        "SET #type = :type, entityType = :entityType, siteId = :siteId, providerId = :providerId, providerSiteId = :providerSiteId, #name = :name, places = if_not_exists(places, :places), seededAt = if_not_exists(seededAt, :now), updatedAt = :now" +
        (seed.location
