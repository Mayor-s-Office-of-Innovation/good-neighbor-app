...
export async function validateDescription({
  ...
  // Escape hatch: when explicitly disabled (e.g. local dev, where the keyword
  // heuristic is too strict), skip the check and accept the description outright so
  // the flow never gets stuck. Off by default; opt in via env. Do NOT set in prod.
  if (process.env.DESCRIPTION_VALIDATION_DISABLED === "true") {
    return {
      accepted: true,
      whatYouCanSee: true,
      whereItIs: true,
